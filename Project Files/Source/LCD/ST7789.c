...
void LCD_Write_Rectangle_Pixel_2(const uint8_t *pSrcR,const uint8_t *pSrcB,const uint8_t *pSrcG,const int by_limit,const int bx_limit) {
	uint32_t b1,b2;
	uint32_t *R_1,*G_1,*B_1;
	R_1=(uint32_t*)pSrcR;
	G_1=(uint32_t*)pSrcG;
	B_1=(uint32_t*)pSrcB;
	GPIO_SetBit(LCD_D_NC_POS);
	// 16 bpp, 5-6-5. Assume color channel data is left-aligned
	for(int j=0;j<by_limit*(bx_limit/4);j++)
	{
		b1=(R_1[j] & 0xf8f8f8f8)|((G_1[j]&0xe0e0e0e0)>>5);
		b2=((G_1[j]&0x1c1c1c1c)<<3)|((B_1[j]&0xf8f8f8f8)>>3);
		for(int i=0;i<4;i++){
		GPIO_Write(b1>>(i*8));
		GPIO_ResetBit(LCD_NWR_POS);
		GPIO_SetBit(LCD_NWR_POS);
		...
		#if ENABLE_PIXEL_HASH	
		// Update pixel_data_hash
		pixel_data_hash ^= (uint8_t)(b1>>(i*8)) ^ (uint8_t)(b2>>(i*8));
		if (pixel_data_hash & 0x01) {
			pixel_data_hash >>= 1;
			pixel_data_hash |= 0x80000000;
		} else {
