               ...
               if (image_info.m_scanType == PJPG_GRAYSCALE) {
                  ...
                  for (by = 0; by < by_limit; by++) {
                     ...
                     //pDst_block += row_pitch;
								  ...
								  PT_T start,end;
								  start.Y = y + mcu_y * image_info.m_MCUHeight;
									start.X = x + mcu_x * image_info.m_MCUWidth;
									end.X=start.X+bx_limit-1;
									end.Y=start.Y+by_limit-1;
								  LCD_Start_Rectangle(&start,&end);
								  LCD_Write_Rectangle_Pixel_2(pSrcR,pSrcB,pSrcG,by_limit,bx_limit);
               }
            }					
         }
      }
